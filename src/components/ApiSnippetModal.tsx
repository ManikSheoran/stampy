...
export const ApiSnippetModal: React.FC<ApiSnippetModalProps> = ({ config, isOpen, onClose }) => {
  ...
  const copiedIcon = (key: string) =>
    copied === key ? <Check size={15} weight="bold" /> : <Copy size={15} />;

  return (
    <div
      className="api-redesign-overlay"
      onClick={onClose}
      role="dialog"
      aria-modal="true"
    ...
    >
      <section className="api-redesign-modal" onClick={(event) => event.stopPropagation()}>
        <header className="api-redesign-header">
          <div className="api-redesign-kicker">
            <span className="api-orb">
              <Globe size={15} weight="bold" />
            </span>
            <span>STAMPYYY / DEVELOPER TOOL</span>
          </div>
          <button className="api-close" onClick={onClose} aria-label="Close API modal">
            <X size={18} />
          </button>
          <h2 id="api-modal-title">
            Put a stamp<br />
            <em>anywhere.</em>
          </h2>
        ...
        <div className="api-redesign-body">
          <div className="api-endpoint-block">
            <div className="api-section-label">
              ...
              <div>
                <a href={svgUrl} target="_blank" rel="noreferrer">
                  SVG <ArrowSquareOut size={13} />
                </a>
                <a href={pngUrl} target="_blank" rel="noreferrer">
                  PNG <ArrowSquareOut size={13} />
                </a>
              </div>
            </div>
            <div className="api-url-row">
              <span className="api-method">GET</span>
              <input readOnly value={pngUrl} aria-label="API endpoint" />
              <button
                type="button"
                onClick={() => copy(pngUrl, 'url')}
            ...
            <div className="api-minimal-row">
              <span>MINIMAL CALL</span>
              <code>{minimalUrl}</code>
            </div>
          </div>
            ...
            <div className="api-section-label">
              ...
              <div className="api-tabs">
                {(['html', 'markdown', 'fetch', 'curl'] as const).map((tab) => (
                  ...
                  >
                    {tab === 'fetch' ? 'JS' : tab === 'markdown' ? 'MD' : tab.toUpperCase()}
                  </button>
                ))}
              </div>
            </div>
          ...
          <div className="api-params">
            <div className="api-section-label">
              ...
              <span>OPTIONAL VALUES ARE GENERATED</span>
            </div>
            <div className="api-param-grid">
              ...
              <span>
                <b>theme</b> colour palette
              </span>
              <span>
                <b>motif</b> artwork style
              </span>
              <span>
                <b>format</b> png / svg
              </span>
        ...
        <footer className="api-redesign-footer">
          <span>GET STARTED WITH A MINIMAL CALL</span>
          <button type="button" onClick={onClose}>
            BACK TO STAMP <ArrowSquareOut size={14} />
          </button>
        </footer>
      </section>
    </div>
